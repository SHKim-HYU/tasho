...
class WorldSimulator:
    def __init__(self, plane_spawn=True, bullet_gui=True, physics_ts=1.0 / 240):
        ...
        self.verbose = True
        self.physics_ts = (
            physics_ts  # Time step for the bullet environment for physics simulation
        )
    ...
    def computeInverseDynamics(self, robotID, positions, velocities, accelerations):
        ...
        jointTorques = p.calculateInverseDynamics(
            robotID, positions, velocities, accelerations
        )
        ...
        if controller_type == "velocity":
            if targetPositions[0] != None:
                p.setJointMotorControlArray(
                    ...
                    velocityGains=np.array([1, 1, 1, 1, 1, 1, 1]),
                )  # , forces = [50]*len(targetVelocities))
            else:
                p.setJointMotorControlArray(
                    robotID,
                    joint_indices,
            ...
            p.setJointMotorControlArray(
                robotID, joint_indices, p.TORQUE_CONTROL, forces=targetTorques
            )
        else:
