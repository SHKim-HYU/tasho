...
if __name__ == '__main__':
	...
	if ocp_control == 'acceleration_resolved':
		...
		control_regularization = {'hard': False, 'expression':tau, 'reference':0, 'gain':1e-5}
	path_pos = {'hard':False, 'type':'Frame', 'expression':fk_vals, 'reference':T_goal, 'rot_gain':1, 'trans_gain':1, 'norm':'L2'}
	...
	if coll_avoid:
		...
		ball_obs = {'center':[0.25, 0.0, 1.1], 'radius':0.1}
		cube = {}
	...
	rot_err = robot.fk(q_sol[-1,:])[6][0:3,0:3]@T_goal[0:3,0:3]
	...
	if coll_avoid:
		...
		obj.add_cube({'position':ball_obs['center'], 'orientation':[0,0,0,1.0]}, scale = ball_obs['radius']*2, fixedBase = True)
