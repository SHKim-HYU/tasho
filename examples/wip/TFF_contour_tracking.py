...
if __name__ == "__main__":
    ...
    if bullet_mpc_nr:
        ...
        if task_index == 0:
            ...
            bullet_world.add_cube(
                {"position": [0.7, 0.0, 0.25], "orientation": [0.0, 0.0, 0.0, 1.0]},
                scale=0.57,
            )
        ...
        # progress variables
        s = tc.create_expression(
            "s", "state", (1, 1)
        )  # Progress variable for the contour tracing task
        ...
        jac_val = jac_fun(q0)
        q_dot_force = cs.mtimes(
            jac_val.T,
            cs.solve(
                cs.mtimes(jac_val, jac_val.T) + 1e-6,
                K * (force_desired - force_measured),
            ),
        )
        q_dot_force_fun = cs.Function(
            "q_dot_force_fun", [q0, force_desired, force_measured], [q_dot_force]
        )
        ...
        angle_limit = cos(
            10 * 3.14159 / 180
        )  # The constraint on the EE angle during contour-tracing
        ...
        angle_constraint = {
            "hard": False,
            "equality": True,
            "expression": dot_prod_ee_workpiece,
            "reference": 1.0,
            "gain": 10.0,
        }  #'upper_limits':-angle_limit}
        ...
        contour_error_slack_con1 = {
            "inequality": True,
            "hard": True,
            "upper_limits": contour_error_slack,
            "expression": fk_vals[0:3, 3]
            - p_des[0:3]
            + np.array([0.005, 0.005, 0.005]),
        }
        contour_error_slack_con2 = {
            "inequality": True,
            "hard": True,
            "expression": -contour_error_slack,
            "upper_limits": fk_vals[0:3, 3]
            - p_des[0:3]
            - np.array([0.005, 0.005, 0.1]),
        }
        # TODO: create a function in task_protoype to add objectives without specifying as constraints
        tc.ocp.add_objective(
            tc.ocp.sum(contour_error_slack[0]) * 5.0
            + tc.ocp.sum(contour_error_slack[1]) * 5.0
            + tc.ocp.sum(contour_error_slack[2]) * 1.0
        )
        contour_error = {
            "lub": True,
            "hard": True,
            "expression": fk_vals[0:3, 3] - p_des[0:3],
            "upper_limits": [0.005, 0.005, 0.005],
            "lower_limits": [-0.005, -0.005, -0.1],
        }
        vel_regularization = {
            "hard": False,
            "expression": q_dot,
        ...
        s_regularization = {
            ...
            "gain": 50,
            "norm": "L1",
        }  # push towards contour tracing #30 for planar contour
        s_dot_regularization = {
            "hard": False,
            "expression": s_dot,
            "reference": 0.3,
            "gain": 0.01 * 100,
        ...
        bullet_world.setController(
            kukaID, "velocity", joint_indices, targetVelocities=[0] * 7
        )
        ...
        mpc_params["params"]["f_des"] = {
            "type": "set_value",
            "value": np.array([0, 0, -20]),
        }
        ...
        def joint_force_compensation(fk, q, force):
            ...
            force_last_link = (
                cs.mtimes(invJointPose[0:3, 0:3], reactionGravVector) * mass_last_link
            )
            force_corrected = force - force_last_link
            force_corrected = cs.mtimes(jointPose[0:3, 0:3], force_corrected)
            return force_corrected

        mpc_params["params"]["f_meas"] = {
            "type": "joint_force",
            "robotID": kukaID,
            "joint_indices": [6],
            "fk": robot.fk,
            "post_process": joint_force_compensation,
        }
        ...
        invJointPose = utils.geometry.inv_T_matrix(
            jointPose
