...
while True:
    ...
    MPC_component.input_ports["port_inp_x00"]["val"] = xd_control_sig
    MPC_component.input_ports["port_inp_y00"]["val"] = yd_control_sig
    MPC_component.input_ports["port_inp_th00"]["val"] = thd_control_sig
    MPC_component.input_ports["port_inp_v00"]["val"] = vd_control_sig
    MPC_component.input_ports["port_inp_w00"]["val"] = wd_control_sig
    MPC_component.input_ports["port_inp_dv00"]["val"] = dvd_control_sig
    MPC_component.input_ports["port_inp_dw00"]["val"] = dwd_control_sig
    MPC_component.input_ports["port_inp_ddv00"]["val"] = ddvd_control_sig
    MPC_component.input_ports["port_inp_ddw00"]["val"] = ddwd_control_sig

    # Find closest point on the reference path compared witch current position
    # index_closest_point = find_closest_point(q_now[:2], ref_path, index_closest_point)
    index_closest_point = find_closest_point([xd_control_sig,yd_control_sig], ref_path, index_closest_point)
    ...
    if obstacle_avoidance==True:
        ...
        dist_obs, closest_obs = find_closest_obstacle([xd_control_sig,yd_control_sig], ref_obs)
